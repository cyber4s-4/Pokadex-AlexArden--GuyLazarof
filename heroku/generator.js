...
const mix = () => {
  const pokemons = JSON.parse(fs.readFileSync(dataPath, { encoding: "utf-8" }));
  const pokefusion = [];
  ...
  for (let i = 0; i < pokemons.length; i++) {
    ...
    for (let j = i + 1; j < pokemons.length; j++) {
      ...
      const newName =
        firstPokemon.name.substr(0, parseInt(firstPokemon.name.length / 2)) +
  ...
  fs.writeFileSync("./data.json", JSON.stringify([...pokemons, ...pokefusion]));
};
